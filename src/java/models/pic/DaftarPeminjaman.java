...
public class DaftarPeminjaman {
    String no_lab, ketua_kegiatan, kontak_ketua, tgl_peminjaman, tgl_mulai; 
    String tgl_berakhir, keterangan, status;
    int level, id_peminjaman;
    database db = null;
    ...
    public List tampilDaftarDalamProses(){
        List<DaftarPeminjaman> data = new ArrayList<DaftarPeminjaman>();
        ResultSet rs = null;
        
        try{
            String sql = "SELECT p.id_peminjaman, l.no_lab, p.ketua_kegiatan, p.kontak_ketua, \n" +
                "p.level, p.tgl_peminjaman, p.tgl_mulai, p.tgl_berakhir,\n" +
                "p.keterangan, p.status_peminjaman\n" +
                "FROM tbl_peminjaman p, tbl_lab l\n" +
                "WHERE p.id_lab = l.id_lab AND p.status_peminjaman NOT LIKE \"Selesai\"";
            rs = db.getData(sql);
            while(rs.next()){
                ...
                dp.setStatus(rs.getString("status_peminjaman"));
    ...
    public List tampilDaftarSelesai(){
        List<DaftarPeminjaman> data = new ArrayList<DaftarPeminjaman>();
        ResultSet rs = null;
        try{
            String sql = "SELECT p.id_peminjaman, l.no_lab, p.ketua_kegiatan, p.level,\n" +
                    "p.tgl_peminjaman, p.keterangan, p.status_peminjaman\n" +
                    "FROM tbl_peminjaman p, tbl_lab l\n" +
                    "WHERE p.id_lab = l.id_lab\n" +
                    "AND p.status_peminjaman LIKE \"Selesai\";";
            rs = db.getData(sql);
            while(rs.next()){
                DaftarPeminjaman dp = new DaftarPeminjaman();
                dp.setId_peminjaman(rs.getInt("id_peminjaman"));
            ...
            System.out.println("Terjadi Kesalahan Saat menampilkan daftar peminjaman" + ex);
        }
        return data;
    }
}